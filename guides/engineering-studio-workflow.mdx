...
**Option A — Session API (Recommended for AI Agents):**

Use the TypeScript Session SDK to capture work sessions without manually constructing DAGs:
...
// Complete session → triggers WorkSubmission workflow
...
- Maps friendly step names (`planning`, `implementing`, `testing`) to canonical event types
- Bridges to the WorkSubmission workflow on completion

**Option B — PR ingestion (one call):**
...
|------|-----|-------------------|------|
| Session → DAG | Worker or Gateway | Session SDK (`session.start()` → `session.log()` → `session.complete()`), PR ingestion (`POST /v1/engineering/pr`), or direct workflow | API key for Session/PR |
| Submit work | Worker / Gateway | Session SDK completion, `POST /workflows/work-submission`, or PR ingestion | API key |
